[CmdletBinding()]
param(
    # NOT the 30-60 minute row in concept docs/scraplify-concept.md section
    # 19.2 - that row is for lightweight discovery-only polling, which this
    # crawl does not do: runJobsGeCrawl refetches every discovered listing's
    # detail page every run (fullCoverage: true), which is section 19.2's
    # "Complete source reconciliation" row instead ("Nightly or weekly").
    # Measured basis (adversarial review, 2026-09-05, round 8): ~5,647
    # listings + ~21 discovery/probe fetches, at jobsGePolicy's 5s
    # crawl-delay and maxConcurrency 1 (src/policies/jobs-ge.ts), is >=
    # 7h50m of fetches alone per run - a 60-minute cadence would just run
    # back-to-back continuously while claiming to be hourly. 1440 (24h) is
    # the conservative end of "nightly." True hourly-ish discovery is what
    # concept section 10.1's incremental discovery would unlock -
    # deliberately not implemented yet (docs/STATUS.md), since jobs.ge's
    # corpus is small enough to fully re-walk on this slower cadence instead.
    [int]$IntervalMinutes = 1440
)

# Registers (or re-registers) a Windows Task Scheduler job that runs the
# jobs.ge crawl on a recurring cadence, per concept section 19.1's "Windows
...
$action = New-ScheduledTaskAction -Execute 'powershell.exe' `
    -Argument "-NoProfile -ExecutionPolicy Bypass -File `"$wrapperScript`" -NodePath `"$nodePath`"" `
