...
**Decided in Phase 8C (2026-09-25):**
- **Maximum bundle age: 72 hours.** Crawls are scheduled daily and a source counts as overdue after 48h, so a bundle older than 72h has missed at least a full day of crawls. Past 36h the health check warns first.
- **Vector tables deferred.** Phase 8A closed lexical-first, since its one candidate model failed the browser size and load gates, so there is no approved model contract to key vectors on. `embedding_models` and `opportunity_embeddings` are not created yet, and the first bundle contract (`lexical-v1`) carries public render metadata, taxonomy terms and provenance without vectors. Every bundle row already carries its `semantic_input_hash`, so embeddings can be added later as incremental work under a new contract name and schema version, with no change to the existing build and publication tables.
