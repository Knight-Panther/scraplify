...
| --- | --- | --- |
| A crawl exits at once with "a run is already in progress" | A previous run crashed and left its `crawl_runs` row `running`. Self-healing is Phase 7B, not built. | Confirm no crawl process is running, then settle that row with the SQL in the error message (by id). The next timer run crawls. |
