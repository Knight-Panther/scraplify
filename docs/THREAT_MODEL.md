...
| XSS via CV-derived text | Evidence snippets, terms and bundle fields render as React text only. Nothing on the path uses `dangerouslySetInnerHTML`. | `rg dangerouslySetInnerHTML` over the CV path finds nothing. **Residual:** a strict CSP (`connect-src 'self'`, `worker-src 'self'`) is Phase 8E hosting work. Nothing here needs `eval` or a remote origin, so it will fit. |
