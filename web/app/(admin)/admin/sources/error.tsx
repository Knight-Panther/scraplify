...
/**
 * This route's own error state, following the same scoped-boundary pattern
 * every database-backed screen here uses — `retry`, not `reset`: `reset()`
 * re-renders the boundary's children without re-fetching, so against a
  ...
  return (
    <main className="w-full px-4 py-8 sm:px-6 sm:py-10">
      ...
      <div className="mt-6 max-w-[var(--measure)] rounded-[var(--radius)] border border-status-held bg-surface px-4 py-4">
        <p className="text-sm">Source health could not be loaded.</p>
