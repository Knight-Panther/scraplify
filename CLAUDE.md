...
`docs/scraplify-concept.md` §30 (amended 2026-09-23) records the accepted direction for a hosted public catalogue plus privacy-first browser CV matching, alongside the local/operator workflow this file already documents — one repository, one codebase, three runtime surfaces (`local`, `public`, `admin`), no fork and no permanent hosted branch. **Phase 8A is merged and Phase 8B (the surface split itself) is in progress** — `docs/STATUS.md`'s current-phase section is the source of truth for exactly how far. Until 8B actually lands its `XTELO_SURFACE` env var, `proxy.ts` and the admin/public layouts, every rule above and below this section still describes the entire current system with no exceptions — but that is changing incrementally as 8B's own stages merge, not on a single cutover date. `change.md` (repository root, kept per its own note at the top) is the full implementation handoff Phase 8A–8E work from.
