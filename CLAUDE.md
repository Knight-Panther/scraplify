...
`docs/scraplify-concept.md` §30 (amended 2026-09-23) records the accepted direction for a hosted public catalogue plus privacy-first browser CV matching, alongside the local/operator workflow this file already documents — one repository, one codebase, three runtime surfaces (`local`, `public`, `admin`), no fork and no permanent hosted branch. **Phases 8A and 8B are merged, so the surface split is live.** `XTELO_SURFACE` selects `local` (the default: everything above still applies unchanged), `public` or `admin`. `web/proxy.ts` and each action's own guard enforce which routes and actions exist on each surface, and four least-privilege database roles (`scripts/sql/phase-8b-*.sql`) exist for deployments. **Phase 8C (the matching bundle) is in progress.** `docs/STATUS.md`'s current-phase section is the source of truth for exactly how far. `change.md` (repository root, kept per its own note at the top) is the full implementation handoff Phase 8A–8E work from.
